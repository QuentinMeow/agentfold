...
The delivered stack is [PR90](https://github.com/QuentinMeow/agentfold/pull/90) below [PR91](https://github.com/QuentinMeow/agentfold/pull/91). `recovery-decisions.md` preserves the adjudications and discarded approaches. Original PRs 88 and 89 are closed only after both replacement code candidates passed their checks. `review-limit.md` separates the unexecuted external review from the completed native evidence. No product change is merged to main.
