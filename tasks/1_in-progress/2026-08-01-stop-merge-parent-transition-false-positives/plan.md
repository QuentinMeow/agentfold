...
- [ ] 1. Claim the task, move it to `1_in-progress`, and resolve its pickup request in one
      coordination commit.
- [ ] 2. Record the pre-fix reproduction in `verification.md`: the merge probe transcript,
...
- [ ] 3. Add `task_status_at_other_parent` beside `task_recorded_at_other_parent` in
...
- [ ] 4. Condition only the transition-table finding in `task_topology_problems` on that
...
- [ ] 5. Add `test_task_admission_accepts_a_merge_inheriting_an_advanced_task` over a real
      two-parent merge, and record its pre-fix verdict.
- [ ] 6. Add `test_task_admission_still_rejects_an_illegal_merge_advance_past_a_sibling`,
...
- [ ] 7. Re-run the merge probe with the fix applied and record its real output.
- [ ] 8. Record a full `--check`, the whole test suite, and every discriminating verdict in
