...
- [ ] 2. Record the design: why bare equality against `github.sha` is the wrong binding,
      which of the sibling job's mechanism is reused, and the completed **Core fit** receipt.
- [ ] 3. Rewrite the `review-state-action-projection` candidate step so a candidate that
...
- [ ] 4. Bound the re-resolution: a fixed attempt count and a fixed delay, both declared in
      the step's `env:` block, with exhaustion failing the job.
- [ ] 5. Extend `automation/tests/test_github_action_projection_workflow.py`: a fixture in
...
- [ ] 6. Record real output in `verification.md`: the full suite, the reconciler, the new
