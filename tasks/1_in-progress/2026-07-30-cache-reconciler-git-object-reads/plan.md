...
- [ ] 2. Separate the unmerged branch's caching from its resolution-evidence rule; confirm
      commit-by-commit that the caching landed before the rule and can be taken alone.
- [ ] 3. Add `read_raw_git_object` plus `parse_raw_git_tree` /
...
- [ ] 4. Consult it from `git_tree_blob_entry` and `git_tree_path_entry`, leaving both
      subprocess paths untouched behind the sentinel.
- [ ] 5. Prove equivalence differentially: the same calls with the reader on and off over
      every path kind in the repository, recorded in `verification.md`.
- [ ] 6. Prove whole-run equivalence and degradation: identical findings with the reader
      forcibly unavailable.
- [ ] 7. Prove the shallow-clone failure path: the discarded branch raises, this change
      reports the same findings and the same exit code as main.
- [ ] 8. Measure `--check` before and after over several runs and record the spread.
- [ ] 9. Full suite and `reconcile.py --check` green, then commit on
