...
- [ ] 2. Split path from fragment in `check_links` so the existing root/local existence
...
- [ ] 3. Add ATX heading extraction and a GitHub slug function to the reconciler — lowercase,
...
- [ ] 6. Add the append-only ledger beside the tool and the verbs that write one durable
...
- [ ] 7. Unit-test the CLI in a new file automation/tests/test_mine_cochange.py against a
...
- [ ] 9. Run the gating experiment and write it down: for the two hottest markdown files in
