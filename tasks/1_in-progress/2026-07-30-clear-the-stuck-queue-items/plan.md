...
- [x] 2. Confirm from Git that the code-span repair really merged: the trailer commit, its
      ancestry on `main`, and the item's own `Done when` clauses.
- [x] 3. Delete the merged code-span repair request together with its reciprocal
      `Queue actions` backlink in one commit, with the reconciler green and no evidence file
      staged.
- [x] 4. Audit task 2026-07-25-fix-handover-projection-code-span-copy against its own
...
- [x] 5. Measure the three stranded merge reviews: ancestry of each bound range, what the
      deletion path reports, and what a `--at-transition merge` replay reports.
- [x] 6. Record the diagnosis in `design.md` with the options that were rejected and why.
- [x] 7. File one canonical `message-queue/needs-human/decisions/` item for the three, link it
      reciprocally from all three task records, and append a worklog entry to each.
- [x] 8. Record real `--check` and full-suite output in `verification.md` and update
