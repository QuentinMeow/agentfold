...
- [ ] 1. Re-verify all fourteen findings against current `main` before any edit — quote both
...
- [ ] 2. Finding 1 — file `message-queue/needs-human/decisions/` item from
...
- [ ] 3. Finding 3 — choose between rewriting the `pair` column of
...
- [ ] 4. Finding 2 — correct the `needs-human/` timing-field grammar in
      `templates/README.md` so it matches the three templates in `templates/queue/`.
- [ ] 5. Findings 4 and 6 — document what the link check actually exempts in
...
- [ ] 6. Findings 7 and 8 — add the two legal status transitions to the `tasks/AGENTS.md`
      lifecycle diagram and make the `open` → `in-repair` claim rule name its real subject.
- [ ] 7. Findings 5 and 10 — remove the dead decision path from `roadmap/current-state.md`
      and repoint the stale citation in
      `2026-08-01-stop-the-merge-ref-recompute-from-failing-a-stack`.
- [ ] 8. Findings 9, 11, 12 and 14 — rewrite `memory/facts/archived-refs-outside-core.md`,
...
- [ ] 9. Finding 13 — correct or route the hard-coded root budget in
      `handbook/principles/progressive-disclosure.md`; the call and its argument go in
      `design.md` either way.
- [ ] 10. Verification — write a real `needs-human` item exactly as the corrected
