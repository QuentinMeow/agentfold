...
- [ ] 1. Reproduce all three closed exits of the claim-before-evidence deadlock on a
      generated retry, and settle the suspected ordinary-agent-request sub-case, recording
      the real reconciler messages
- [ ] 2. Reproduce the retry garbage-collection escape for both `stale-task` and
...
- [ ] 3. Stop a committed agent claim from freezing `Resolution evidence`: move the field
...
- [ ] 4. Register `stale-task` in `CHECKS` and make the runner deduplicate checks by
      function identity, so each finding is still reported exactly once
- [ ] 5. Name the retry garbage-collection predicate and tie it to what
      `queue_deletion_problem` will certify, so the two cannot drift apart again
- [ ] 6. Emit `**Resolution evidence:**` from `retry_text`, add it from
...
- [ ] 7. Add regression tests: each closed exit, the live evidence establishment, the
...
- [ ] 8. Full suite green and `--check` 0 findings, both recorded in `verification.md`
