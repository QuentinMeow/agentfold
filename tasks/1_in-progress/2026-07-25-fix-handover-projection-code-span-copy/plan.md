...
- [ ] 2. Normalise both sides of the handover projection's context comparison through
      `render_inline_code`, leaving the two-element rendered-HTML guard intact.
- [ ] 3. Measure whether dropping the code-span blanking changes any existing `needs-agent`
      entry verdict, on the newly-added-handover path and over a `--range` of recent history.
- [ ] 4. Take the measured branch: keep the fix unguarded if no existing record changes
      verdict, otherwise scope it to `needs-human` and file the agent-side hole separately.
- [ ] 5. Add regression tests in `automation/tests/test_reconcile_queue.py` covering the raw
...
- [ ] 6. Record the chosen normalisation and both rulings in `design.md` with a complete
      `## Core fit` receipt.
