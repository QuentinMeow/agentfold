...
Local evidence uses the candidate's regular-file modes and bytes, with lexical file-relative or root-relative resolution. Markdown headings and bounded line ranges select source text. Quotation comparison preserves identifiers and case while permitting presentation-only whitespace/emphasis and ordered elisions; all quote lengths are checked. Missing or unsupported local sources produce advisory findings, and external source content is not machine verified. The explicit no-source sentence never substitutes for a local review target.
