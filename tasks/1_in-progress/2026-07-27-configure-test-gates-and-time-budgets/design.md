...
work, budget filing, receipt/report/marker validation, and final accounting finish before one
gate outcome is frozen. Publication has a separate command outcome. The receipt and pass report
are projected afterward, followed by stdout. Only after those projections succeed is the atomic
commit marker that attests them written last. A self-report cannot include its own completed
projection. That external I/O may delay
...
The first implementation put the full-suite cache and time budgets in place, then two early
adversarial review cycles repaired environment binding, process cleanup, per-test projections,
and an unsafe proposed publisher. A later security review established that those defenses still
did not provide controlled completion, which caused the authorized manual-only replan.

The revision-bound merge panel for `e530c428..64490ab5` then voted 0 approve and 5 block. Its six
combined blockers were: reject automatic authority before imports; execute one immutable closure;
restore non-enforcing PR admission diagnostics while retiring the old check; freeze the terminal
report; include candidate-only test namespaces; and close obsolete human asks.

Three focused repair reviews followed. Round 1 fixed a pass report surviving a receipt failure,
a nested-namespace crash, a successful exit with a detached child, mutable `PYTHONPATH` source
bytes, and false drift caused by sealed file modes. Round 2 fixed unavailable process discovery
being treated as an empty successful scan and a receipt surviving failed rollback. Round 3 fixed
a mutable per-component frozen index and incomplete proof of the isolated child interpreter.
The current candidate contains those repairs. Focused evidence is recorded in `verification.md`;
it does not substitute for the still-pending exact final/full run and fresh merge review.
