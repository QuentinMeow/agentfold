...
- [x] 9. Replace the invalidated test-only approval of `21d5a24` with the staged cleanup
      ...
      before cleanup commit `c78a8d6`. Commit `7d580d3` then refreshed that immutable helper for
      the repaired `a27b85…` runner and passed its own exact final gate; the production pivot's
      exact final remains part of open step 12.
