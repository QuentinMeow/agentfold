...
- [ ] 1. Reproduce the anchor-hole before-state in a detached worktree at `e52f68e^`: one
...
- [ ] 2. Run the same fixture at today's tip and paste the `link-check` finding naming the
      missing path
- [ ] 3. Run a fixture whose path exists but whose fragment does not, and paste the second,
      distinct `link-check` finding naming the fragment
- [ ] 4. Measure `agents-budget` against `automation/AGENTS.md` and paste both the budget
      and the real line count
- [ ] 5. Append the four sections to the mining task's `verification.md`, leaving its
      existing 906 lines byte-identical (digest + `git diff` prove it)
- [ ] 6. Re-file the scratch-discipline contradiction as a `needs-agent` request from
      `templates/queue/request.md`
- [ ] 7. Write this task's own `verification.md`: the staged `reconcile.py --check` run and
