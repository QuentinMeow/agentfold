...
**Queue actions:** `message-queue/needs-agent/requests/non-blocking-pick-up-universal-guard-mode-configuration.md`; `message-queue/needs-human/reviews/future-blocking-review-detector-failure-state.md`; `message-queue/needs-human/reviews/future-blocking-review-guardrail-authority-boundary.md`; `message-queue/needs-human/reviews/future-blocking-review-revised-assurance-profile-scope-and-egress.md`; `message-queue/needs-human/reviews/future-blocking-review-sensitive-data-recovery.md`
...
pretending disabled controls provide evidence. The system derives assurance per
obligation and scope from observed coverage, detector health, and enforcement rather
than accepting a configured or agent-written label. Capability sandboxing and broader
non-Git boundaries are not part of this task.

## Acceptance criteria

- [ ] A canonical template defines versioned obligation/scope/detector/trigger guard
      bindings with one mode each, and the active repository config links to that
      template instead of inventing another schema.
...
- [ ] Output distinguishes disabled, not-triggered, clean, finding, incomplete, and
      error states so a weaker mode cannot masquerade as stronger evidence.
- [ ] A derived report lists observed coverage, skips, limitations, health, and
      enforcement per obligation and scope; uninvoked manual guards contribute no
      current evidence, and an agent cannot self-declare stronger assurance.
- [ ] No filesystem/network sandbox, controlled-egress mechanism, or agent-specific
