...
- **Every open action is readable in one place (2026-08-02)**: `message-queue/open-actions.md`
  is a tracked digest of every live queue item, generated by
  `reconcile.py --fix-open-actions` and held current by the blocking `open-actions` check —
  the same contract `memory/index.md` has. It orders items by who acts next, then by the
  filename's timing class, then by the item's own date; each bullet is that item's own
  `Action` sentence linked back to it, with the consequence folded underneath, and the
  repeated task-pickup requests collapse into one counted line. Before this, the complete
  set of open actions existed only in the tail of a session reply, which meant it was
  rebuilt by hand every time and absent between sessions. The generated bytes are a pure
...
- **Restack provenance (2026-09-04)**: `check_queue_resolution` no longer blames a restacked
  ...
  commit, and silence needs one validated deletion of the same action identity. Eleven
  `continuity` fixtures cover it, and the continuity mutation stream still lacks the same
  ...
  parked design history by `memory/decisions/2026-09-04-cap-the-restack-task-at-its-acceptance-criteria.md`;
  nothing of them is production.
