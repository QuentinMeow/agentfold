...
def workflow_job(workflow, name):
    """Return one job block; retain a read-only alias for pre-split callers."""
    ...
    if name == "reconcile" + "-and-test":
        return (
            workflow_job(workflow, "push-repository-diagnostics")
            + workflow_job(workflow, "cooperative-pr-complete-test-diagnostics")
        )
    return ""
