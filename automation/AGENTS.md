...
|-------|--------------|------|
| `check_action_projection.py` | binds provider asks and task actions to live canonical queue items for the selected next actor; `--pull-request-body-shape` also reports the readability rules `templates/pull-request.md` makes visible — sections present and in order, summary length — as advisory lines that never change its exit status | PR/provider boundary, on demand |
...
| `reconcile/reconcile.py` | checks every harness invariant, streaming findings as they are produced so a later failure cannot discard them; every finding is `blocking` or, for the ids in `ADVISORY_CHECKS`, `advisory` — advisory findings print with an `(advisory)` marker and are counted separately, but only blocking ones exit 1. Most of that tier is age-driven (`memory-expiry`, `roadmap-fresh`, `stale-queue`, `stale-task`), so an unchanged clean tree never fails on a calendar date; `explanation-shape` is there for the other reason, that a checker can see a readability rule's shape and not its intent, so it never refuses a commit; `--fail-on-advisory` opts a maintenance run into failing on them too, and a check that cannot run at all exits 2 with one line naming the file or check, never exit 1; existence gates read the Git index rather than `Path.is_file()`, so deleting a worktree copy cannot hide a staged violation; `--file-retries` turns findings into blocking repair items in `message-queue/needs-agent/retries/` and garbage-collects fixed ones; `--word-count` prints words before the answer line against `HUMAN_ATTENTION_WORD_BUDGET` for any file, committed or not, so the format's one threshold is checkable before the commit rather than only by being refused; `--fix-index` regenerates `memory/index.md`, `--fix-queue-fold` re-emits the `## For the record` fold the three human queue templates ship — the one raw-HTML shape `human-attention` admits, whose nine rules `fold-shape` holds, whose column-0 field lines `record-swallow` holds by position in the record region rather than by key name, whose hard breaks the advisory `queue-render` reports, and beside which `queue-frozen-skeleton` refuses a live-item edit that changes raw bytes the action identity cannot see — and `--fix-open-actions` regenerates `message-queue/open-actions.md`, the one ordered digest of every live queue item — actor, then filename timing, then the item's own date — which the blocking `open-actions` check then holds current, and which renders no lateness because tracked bytes that depended on today would fail a tree nobody touched | pre-commit (`--check`), CI, on demand |
...
| `integrate.py` | screens a landing set before any of it merges: `plan` pins the trunk and every leg to a full object id, refuses a leg the trunk has moved past or one that drifted from an earlier manifest, and builds the pairwise conflict matrix — feature-probing `git merge-tree --write-tree` and falling back to a real merge in a scratch worktree, because an older Git reads those words as a revision and would report every pair as colliding; `build` replays the set into a detached worktree under `tmp/`, per leg running the staged test lane against the merge before it is committed, then committing it, because the reconciler binds `--range` to the committed candidate and refuses staged changes, and running the reconciler at the merge transition over that leg's range, stopping at the first failing leg and naming the commits that bracket it, with the full suite once at the end; `verify` re-pins every ref after the real merges land, fails on drift, and asserts the new trunk carries the tree the build produced | before merging a set, on demand |
...
- Retry filing preserves claimed/rejected content on rerun; garbage collection removes only exact generator identities whose named finding cleared.
  Active legacy output may be migrated, but untrusted legacy lookalikes are never garbage-collected.
